...
{
    MPI_Send(matrix->M, matrix->N * sizeof(int), MPI_BYTE, dest, 0, MPI_COMM_WORLD);
}

void recv_matrix1D(Matrix1D *matrix)
{
    MPI_Recv(matrix->M, matrix->N * sizeof(int), MPI_BYTE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
